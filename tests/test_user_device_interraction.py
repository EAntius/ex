...
class TestNipKeyStore(TimedTestCase):
    ...
    def setUp(self):
        ...
        self.central.register_machine(8)
        self.central.register_machine(9)
        self.central.register_machine(9)
        self.central.register_machine(10)
        self.central.register_machine(10)
    ...
    def tearDown(self):
        ...
        self.central = None
        elapsed = time.perf_counter() - start
        print(f"[TIME] tearDown (NipKeyStore cleanup): {elapsed:.6f}s", flush=True)

    def test_user_verification(self):
        for i in range(0,5,2):
            machine1 = self.central.machines[i]
            machine2 = self.central.machines[i+1]
            worker_nfc = self.central.service_workers[0]
            proof1 = machine1.keytree.create_proof()
            proof2 = machine2.keytree.create_proof()
            self.central.delegate_machine_to_worker(i, 0)
            self.central.delegate_machine_to_worker(i+1, 1)
            for _ in range(0, 100):
                self.assertTrue(
                    self.timed(
                        f"machine.authenticate_device (size={8+i/2})",
                ...
                self.assertTrue(
                    self.timed(
                        f"machine2.authenticate_device (size={8+i/2})",
                ...
                self.assertTrue(
                    self.timed(
                        f"machine.authenticate_worker (size={8+i/2})",
    ...
    def test_wrong_user_denial(self):
        for i in range(0,5,2):    
            ...
            for _ in range(0, 100):
                ...
                self.central.remove_worker_from_machine(i, 0)
                self.central.remove_worker_from_machine(i+1, 1)
                
                self.assertFalse(
                    self.timed(
                        f"machine.authenticate_worker denial (size={8+i/2})",
                        machine1.authenticate_worker,
                        worker_nfc,
                    )
                )
    ...
    def test_false_user_identity_denial(self):
        for i in range(0,5,2):
            ...
            for _ in range(0, 100):
                ...
                self.assertFalse(
                    self.timed(
                        f"machine.authenticate_device denial (size={8+i/2})",
                        machine1.authenticate_worker,
                        fake_nfc,
                    )
                )
